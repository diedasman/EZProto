...
class UserProfile:
    ...
    def __post_init__(self) -> None:
        ...
        self.default_output_directory = _clean_output_directory(self.default_output_directory)
...
class AppState:
    ...
    def from_dict(cls, data: dict[str, Any]) -> AppState:
        ...
        return cls(
            ...
            default_output_directory=_clean_output_directory(
                str(data.get("default_output_directory", ""))
            ),
            theme=(str(data.get("theme", DEFAULT_THEME_NAME)) or DEFAULT_THEME_NAME).strip()
...
def default_output_directory(base_path: Path | str | None = None) -> Path:
    return data_directory(base_path)


def machine_user_name() -> str:
    home = Path.home().expanduser()
    return home.name or str(home)


def machine_user_slug() -> str:
    return slugify(machine_user_name())
...
def load_app_state(base_path: Path | str | None = None) -> AppState:
    ...
    return _migrate_legacy_user_state(state, data, base_path)
...
def update_app_state(
    ...
    *,
    ...
    if default_output_directory is not None:
        state.default_output_directory = _clean_output_directory(default_output_directory)
    if theme is not None:
        state.theme = (str(theme) or DEFAULT_THEME_NAME).strip() or DEFAULT_THEME_NAME
    if boards is not None:
        state.boards = _clean_boards(boards)
...
def _app_state_has_content(path: Path) -> bool:
    ...
    return any(
        (
            ...
            str(data.get("default_output_directory", "")).strip(),
            str(data.get("theme", "")).strip(),
            bool(_clean_boards(data.get("boards", {}))),
...
def _migrate_legacy_user_state(
    ...
    legacy_user_slug = str(raw_data.get("last_user_slug", "")).strip() or state.last_user_slug
    if not legacy_user_slug:
        return state

    profile = load_user_profile(legacy_user_slug, base_path)
    if profile is None:
        return state

    mutated = False

    if not str(raw_data.get("default_output_directory", "")).strip():
        state.default_output_directory = profile.default_output_directory
        mutated = True
    if not str(raw_data.get("theme", "")).strip():
        state.theme = profile.theme
        mutated = True
    if not raw_data.get("boards") and profile.boards:
        state.boards = dict(profile.boards)
        mutated = True
    if not str(raw_data.get("last_board_name", "")).strip() and profile.last_generated_board_name:
        state.last_board_name = profile.last_generated_board_name
        mutated = True
    if not raw_data.get("last_board_details") and profile.last_generated_board_details:
        state.last_board_details = dict(profile.last_generated_board_details)
        mutated = True
