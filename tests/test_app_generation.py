...
from ezproto.storage import APP_DATA_ENV_VAR_NAME, DEFAULT_THEME_NAME, load_app_state, update_app_state
    ...
    async def test_app_generates_pcb_and_gerbers_for_project_output_user(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with mock.patch.dict(
                ...
                self.assertEqual(saved_state.default_output_directory, str(output_root))
                self.assertIn(board_name, saved_state.boards)
                self.assertTrue(saved_state.boards[board_name]["zip_generated"])
                self.assertEqual(saved_state.boards[board_name]["zip_archive"], str(archive_path))
    ...
    async def test_app_respects_drill_and_zip_export_options(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.dict(
                ...
                saved_state = load_app_state()
                board_details = saved_state.boards[board_name]
