...
NORMALIZED_BOARDS_PATH = str(Path("C:/Boards").expanduser())
...
class UserProfileStorageTests(unittest.TestCase):
    def test_user_profiles_round_trip_as_json_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(loaded.default_output_directory, NORMALIZED_BOARDS_PATH)
...
class AppStateTests(unittest.TestCase):
    def test_update_app_state_tracks_last_user_and_logs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(state.default_output_directory, NORMALIZED_BOARDS_PATH)
            self.assertEqual(state.theme, "gruvbox")
            self.assertIn("Demo Board", state.boards)
            ...
            self.assertEqual(loaded_state.default_output_directory, NORMALIZED_BOARDS_PATH)
            self.assertEqual(loaded_state.theme, "gruvbox")
            self.assertIn("Demo Board", loaded_state.boards)
            ...
            self.assertEqual(raw["default_output_directory"], NORMALIZED_BOARDS_PATH)
            self.assertEqual(raw["theme"], "gruvbox")
            self.assertIn("Demo Board", raw["boards"])
...
class DefaultStorageLocationTests(unittest.TestCase):
    ...
    def test_default_storage_migrates_legacy_repo_data_once(self) -> None:
        with tempfile.TemporaryDirectory() as legacy_dir, tempfile.TemporaryDirectory() as target_dir:
            ...
            self.assertEqual(state.default_output_directory, NORMALIZED_BOARDS_PATH)
            self.assertEqual(state.theme, DEFAULT_THEME_NAME)
